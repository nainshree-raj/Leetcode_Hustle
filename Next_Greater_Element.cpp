...
then the answer for this query is -1. ANSWER FORMAT:- Return an array ans of length nums1.length such that ans[i] is the next greater element as 
described above.
...
Constraints: 1 <= nums1.length <= nums2.length <= 1000 0 <= nums1[i], nums2[i] <= 104 .
All integers in nums1 and nums2 are unique. All the integers of nums1 also appear in nums2.
